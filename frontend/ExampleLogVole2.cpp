...
namespace osuCrypto
{
    namespace
    {
        const std::vector<std::string> logvole2Tags{ "lv2", "logvole2", "LogVole2" };

#if defined(ENABLE_LOGVOLE)
        u32 logVole2CivoleExamplePlaintextModulusBits(const CLP& cmd)
        {
            const auto plaintextModulusBits = cmd.getOr("p", 55);
            if (plaintextModulusBits < 2 || plaintextModulusBits > 61)
                throw std::runtime_error("LogVole2 CI-VOLE example requires -p in [2, 61]");
            return static_cast<u32>(plaintextModulusBits);
        }

        std::vector<u64> makeLogVole2CivoleExampleX(u64 w, u64 modulus)
        {
            std::vector<u64> x(w);
            for (u64 i = 0; i < w; ++i)
                x[i] = (3 * i + 5) % modulus;
        ...
        void checkLogVole2CivoleExampleRelation(
            span<const u64> x,
            u64 delta,
            span<const u64> keys,
            span<const u64> macs,
            u64 modulus)
        {
            if (keys.size() != x.size() || macs.size() != x.size())
                throw std::runtime_error("LogVole2 CI-VOLE example returned an unexpected output size");
            ...
            for (u64 i = 0; i < x.size(); ++i)
            {
                ...
                if (macs[i] != expected)
                    throw std::runtime_error("LogVole2 CI-VOLE example relation check failed");
            }
        }

        void LogVole2_Civole_local_example(int numVole, int numThreads, std::string tag, const CLP& cmd)
        {
            using namespace LogVole2;

            if (numVole == 0)
                numVole = 16;

            if (numVole < 0)
                throw std::runtime_error("LogVole2 CI-VOLE example requires non-negative -n");

            CivoleSender sender{};
            CivoleReceiver receiver{};
            const auto plaintextModulusBits = logVole2CivoleExamplePlaintextModulusBits(cmd);
            ...
            if (delta == 0)
                throw std::runtime_error("LogVole2 CI-VOLE example requires nonzero delta modulo p");

            auto x = makeLogVole2CivoleExampleX(w, modulus);
            auto offlineSockets = coproto::LocalAsyncSocket::makePair();

            Timer timer;
            const auto begin = timer.setTimePoint("logvole2.local.begin");
            ...
            checkLogVole2CivoleExampleRelation(x, delta, b, a, modulus);

            const auto end = timer.setTimePoint("logvole2.local.end");
    ...
    void LogVole2_Civole_example(Role role, int numVole, int numThreads, std::string ip, std::string tag, const CLP& cmd)
    {
#if defined(ENABLE_LOGVOLE) && defined(COPROTO_ENABLE_BOOST)
        using namespace LogVole2;
        ...
        Timer timer;
        const auto begin = timer.setTimePoint("logvole2.begin");
        ...
        if (role == Role::Sender)
        {
            ...
            const auto end = timer.setTimePoint("logvole2.end");
            const auto milli = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count();
            lout << tag
    ...
    {
        if (!cmd.isSet(logvole2Tags))
            return false;

        if (cmd.hasValue("r"))
            return runIf(LogVole2_Civole_example, cmd, logvole2Tags);
        ...
        LogVole2_Civole_local_example(n, t, logvole2Tags.back(), cmd);
#else
