...
| Rewrite LogVole libOTe integration shell | 25% | In progress | Added fresh lower-layer files under `osuCrypto`; implemented standalone message encoding/decoding and fresh SEAL-backed ring/RNS ops with tests, with no aliases or includes into the frozen `loglabel` reference. |
