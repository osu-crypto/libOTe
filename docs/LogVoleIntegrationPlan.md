...
| Rewrite LogVole libOTe integration shell | 35% | In progress | Added fresh lower-layer files under `osuCrypto`; implemented standalone message encoding/decoding, fresh SEAL-backed ring/RNS ops, and fresh LENC enc/digest/eval ops with tests, with no aliases or includes into the frozen `loglabel` reference. |
