...
| Add coproto protocol entrypoints | 70% | In progress | Added fresh `Sender`/`Receiver` coroutine wrappers over `coproto::Socket&` for key-derive, shrink/expand offline setup, deterministic shrink/expand online expansion, and recursive LogVole offline/online. Remaining work is to audit clean-ot communication counters/reuse semantics and then port CI-VOLE-facing exchanges. |
