...
| Rewrite LogVole libOTe integration shell | 12% | In progress | Added fresh lower-layer skeleton files under `osuCrypto`; implemented standalone LogVole message encoding/decoding with malformed-payload tests, with no aliases or includes into the frozen `loglabel` reference. |
