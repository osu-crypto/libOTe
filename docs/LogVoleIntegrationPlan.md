...
| Rewrite LogVole libOTe integration shell | 70% | In progress | Added fresh lower-layer files under `osuCrypto::LogVole`; implemented standalone message encoding/decoding, fresh SEAL-backed ring/RNS ops, fresh LENC enc/digest/eval ops, fresh key-derive core request/response math, fresh shrink/expand deterministic/full-noise/denoise core math, and the first sender/receiver coproto key-derive entrypoints with no aliases or includes into the frozen `loglabel` reference. |
...
| Add coproto protocol entrypoints | 20% | In progress | Added fresh `Sender`/`Receiver` key-derive coroutine wrappers over `coproto::Socket&` plus a local-socket algebraic relation test. Shrink/expand still needs coroutine entrypoints before the old callback stack can be dropped. |
