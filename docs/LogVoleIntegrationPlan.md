...
| Rewrite LogVole libOTe integration shell | 97% | In progress | Added fresh lower-layer files under `osuCrypto::LogVole`; implemented standalone message encoding/decoding, fresh SEAL-backed ring/RNS ops, fresh LENC/LHE enc/digest/eval ops, fresh key-derive core request/response math, fresh shrink/expand deterministic/full-noise/denoise core math, root wrapper setup/digest/response helpers, top-level root local sender/receiver helpers, clean-ot-profile two-/three-level local recursive flow, local sender precompute/cache, repeated-online, internal setup reuse without a reused offline message, recursive gadget-input subproblem coverage, CI-VOLE CRT/SID wrapper, macoro-backed hot chunk parallelism, async sender precompute/service overlap, and sender/receiver coproto entrypoints for key-derive, shrink/expand, recursive LogVole offline/online, and CI-VOLE public API flows with no aliases or includes into the frozen `loglabel` reference. Continue as a mechanical clean-ot translation, not a protocol redesign. |
...
| Add coproto protocol entrypoints | 92% | In progress | Added fresh `Sender`/`Receiver` coroutine wrappers over `coproto::Socket&` for key-derive, shrink/expand offline setup, deterministic shrink/expand online expansion, recursive LogVole offline/online, and CI-VOLE public API flows. Recursive and CI flows now use deterministic `Socket::fork()` lanes rather than importing clean-ot networking, and sender online overlaps service with precompute. Remaining work is to audit clean-ot communication counters/reuse semantics and decide which benchmark/example surfaces belong in libOTe. |
