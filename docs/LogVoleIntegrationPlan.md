...
| Rewrite LogVole libOTe integration shell | 42% | In progress | Added fresh lower-layer files under `osuCrypto`; implemented standalone message encoding/decoding, fresh SEAL-backed ring/RNS ops, fresh LENC enc/digest/eval ops, and fresh key-derive core request/response math with tests, with no aliases or includes into the frozen `loglabel` reference. |
