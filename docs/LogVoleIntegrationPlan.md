...
| Rewrite LogVole libOTe integration shell | 55% | In progress | Added fresh lower-layer files under `osuCrypto::LogVole`; implemented standalone message encoding/decoding, fresh SEAL-backed ring/RNS ops, fresh LENC enc/digest/eval ops, fresh key-derive core request/response math, and fresh shrink/expand deterministic core math with tests, with no aliases or includes into the frozen `loglabel` reference. |
