...
TEST(LencOps, EncShapeAndDeterminism)
{
    ...
    ASSERT_TRUE(enc1) << enc1.message();

    auto enc2 = lenc_enc(ctx, s, params.tau, params.gadget_log_base, 0xABCDEF01u);
    ASSERT_TRUE(enc2) << enc2.message();

    EXPECT_EQ(enc1.value().r.size(), params.mu);
    EXPECT_EQ(enc1.value().lacct.width_padded, 4u);
    EXPECT_EQ(enc1.value().lacct.levels, 2u);
    EXPECT_EQ(enc1.value().lacct.ct.rows, enc1.value().lacct.levels * enc1.value().lacct.width_padded);
    EXPECT_EQ(enc1.value().lacct.ct.cols, 2u * params.tau);

    expect_batch_equal(enc1.value().r, enc2.value().r);
    expect_tensor_equal(enc1.value().lacct.ct, enc2.value().lacct.ct);
}

TEST(LencOps, DigestDeterministic)
{
    const auto params = make_params();
    ...
    ASSERT_TRUE(digest1) << digest1.message();

    auto digest2 = lenc_digest(ctx, x, params.tau, params.gadget_log_base);
    ASSERT_TRUE(digest2) << digest2.message();

    expect_poly_equal(digest1.value(), digest2.value());
}

TEST(LencOps, EvalMatchesRmulDigestMinusSmulX)
{
    const auto params = make_params();
    ...
    ASSERT_TRUE(eval) << eval.message();

    ASSERT_EQ(eval.value().size(), params.mu);
    for (std::size_t i = 0; i < params.mu; ++i)
    {
        auto r_mul_digest = ring_multiply(enc.value().r[i], digest.value(), ctx);
        ASSERT_TRUE(r_mul_digest) << r_mul_digest.message();

        auto s_mul_x = ring_multiply(s[i], x[i], ctx);
        ASSERT_TRUE(s_mul_x) << s_mul_x.message();
...
TEST(LencOps, EvalRejectsMuMismatch)
{
    const auto params = make_params();
    ...
    ASSERT_FALSE(eval);
    EXPECT_EQ(eval.error(), comm::protocol_errc::config_error);
}
