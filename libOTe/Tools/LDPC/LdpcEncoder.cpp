...
namespace osuCrypto
{
	...
	namespace details
	{
		...
		void DiagInverter::mult(const SparseMtx& y, SparseMtx& x)
		{
			...
			for (u64 i = 0; i < colSizes.size(); ++i)
			{
				...
				x.mCols[i] = end - iter ? SparseMtx::Col(span<u64>(iter, end)) : SparseMtx::Col{};
				iter = end;
			}
			...
			for (u64 i = 0; i < rowSizes.size(); ++i)
			{
				...
				x.mRows[i] = SparseMtx::Row(iter != end ? span<u64>(iter, end) : span<u64>{});
				iter = end;
			}
		...
		void SilverLeftEncoder::init(u64 rows, std::vector<double> rs, u64 rep)
		{
			mRows = rows;
			mWeight = rs.size();
			assert(mWeight > 4);
			...
			for (u64 i = 0; i < mWeight; ++i)
			{
				...
				while (
					//(rep && reps[mYs[i] % rep]) ||
					(rep && mYs[i] % rep) ||
					s.insert(mYs[i]).second == false)
				{
					mYs[i] = (mYs[i] + 1) % rows;

					if (++trials > 1000)
					{
						std::cout << "these ratios resulted in too many collisions. " LOCATION << std::endl;
						throw std::runtime_error("these ratios resulted in too many collisions. " LOCATION);
					}
				}
		...
		void SilverLeftEncoder::init(u64 rows, SilverCode code, u64 rep)
		{
			auto weight = code.weight();
			switch (weight)
			{
			case 5:
				init(rows, { { 0, 0.372071, 0.576568, 0.608917, 0.854475} }, rep);
				...
				break;
			case 11:
				init(rows, { { 0, 0.00278835, 0.0883852, 0.238023, 0.240532, 0.274624, 0.390639, 0.531551, 0.637619, 0.945265, 0.965874} }, rep);
		...
		void SilverLeftEncoder::getTransPoints(PointList& points)
		{
		...
		SparseMtx SilverLeftEncoder::getTransMatrix()
		{
			PointList points(mRows, 2 * mRows);
			getTransPoints(points);
			return points;
		}
		...
		std::vector<SparseMtx> SilverRightEncoder::getTransMatrices()
		{
			std::vector<SparseMtx> ret;ret.reserve(mRows);
			auto colOffset = 0;
			auto rr = mRows;
		...
		SparseMtx SilverRightEncoder::getTransMatrix()
		{
			auto Es = getTransMatrices();
			SparseMtx E = Es.back();
			//std::cout << "Eb" << E << std::endl;
			for (u64 i = Es.size() - 2; i < Es.size(); --i)
			{
				E = E * Es[i];
			}
	...
	void tests::LdpcComposit_RegRepDiagBand_encode_test()
	{
		...
		for (auto code : { SilverCode::Weight5 , SilverCode::Weight11 })
		{
			...
			//{
			//	gVerbose = false;
			//	details::SilverRightEncoder rr;
			//	rr.init(rows, code, true);
			//	auto RR = rr.getMatrix();
			//	std::cout << "R\n" << RR << std::endl;
			//	gVerbose = false;
			//}
