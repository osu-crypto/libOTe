...
namespace osuCrypto
{
	...
	void tests::LdpcComposit_RegRepDiagBand_encode_test()
	{
		...
		for (auto code : { LdpcDiagRegRepeaterEncoder::Weight5 , LdpcDiagRegRepeaterEncoder::Weight11 })
		{
			...
			auto g = enc.mR.gap(code);
			auto d1 = enc.mR.mOffsets[0] + g;
			auto d2 = enc.mR.mOffsets[1] + g;

			//u64 rowIdx = 0;
			//for (auto row : enc.mR.diagMtx_g16_w5_seed1_t36)
			//{
			//	std::set<u64> s;
			//	std::cout << "{{";
			//	for (auto c : row)
			//	{
			//		auto c2 = (c + 16 - 1 - rowIdx) % 16;
			//		s.insert(c2);
			//	}
			//	for(auto c2 : s)
			//		std::cout << int(c2) <<", ";
			...
			//std::cout << std::endl;
			//rowIdx = 0;
			//for (auto row : enc.mR.diagMtx_g32_w11_seed2_t36)
			//{
			//	std::cout << "{{";
			//	std::set<u64> s;
			//	for (auto c : row)
			//	{
			//		auto c2 = (c + 32 - 1 - rowIdx) % 32;
			//		s.insert(c2);
			//	}
			//	for(auto c2 : s)
			//		std::cout << int(c2) << ", ";
			...
			for (u64 i = 0; i < R.cols() - g; ++i)
			{
				...
				auto expSize = enc.mR.weight(code);
