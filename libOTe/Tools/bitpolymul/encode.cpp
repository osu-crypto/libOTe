...
//            }
//            for (u64 j = 0; j < 128; j++) rfx_128[i * 256 + j] = temp128[j * 2];
//            for (u64 j = 0; j < 128; j++) rfx_128[i * 256 + 128 + j] = temp128[j * 2 + 1];
//        }
//    }
//
...
//            // truncated FFT
//            //for(u64 j=0;j<256;j++) temp64[j] = bitmatrix_prod_64x64_M4R( beta_mul_32_m4r , temp64[j] );
//            for (u64 j = 0; j < (64 * 4 / 32); j++) bitmatrix_prod_64x64_4R_b32_avx2((uint8_t*)(&temp64[32 * j]), beta_mul_32_bm4r, (uint8_t*)(&temp64[32 * j]));
//            for (u64 j = 0; j < 64; j++) rfx[i * 256 + j] = temp64[j * 4];
//            for (u64 j = 0; j < 64; j++) rfx[i * 256 + 64 + j] = temp64[j * 4 + 1];
//            for (u64 j = 0; j < 64; j++) rfx[i * 256 + 128 + j] = temp64[j * 4 + 2];
//            for (u64 j = 0; j < 64; j++) rfx[i * 256 + 192 + j] = temp64[j * 4 + 3];
...
//
    ...
    void decode_64(uint64_t* rfx, const uint64_t* fx, u64 n_fx)
    {
        ...
        for (u64 i = 0; i < num; i++) {
            ...
            // truncated iFFT
            //for(u64 j=0;j<64*4;j++) temp64[j] = bitmatrix_prod_64x64_M4R( i_beta_mul_32_m4r , temp64[j] );
#if 0
            for (u64 j = 0; j < (64 * 4 / 32); j++) bitmatrix_prod_64x64_4R_b32_avx2((uint8_t*)(&temp64[32 * j]), i_beta_mul_32_bm4r, (uint8_t*)(&temp64[32 * j]));
#else
