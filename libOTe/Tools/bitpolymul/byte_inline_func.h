...
namespace bpm {
    //static inline
    //    bool byte_is_zero(span<const u64>vec) 
    //{
    //    u64 r = 0; 
    //    for (const auto& v :vec) r |= v;
    //    return (0xff && r) == 0; 
    //}

    //static inline
    //    void byte_xor(span<u64> v1, span<const uint64_t> v2) 
    //{
    //    Expects(v1.size() == v2.size());
    //    for (u64 i = 0; i < v1.size(); i++) v1[i] ^= v2[i]; 
    //}


    //static inline
    //    void u64_rand(span<u64> vec, oc::PRNG& prng) 
    //{ 
    ...
    //static inline
    //    void byte_rand(span<u8> vec, oc::PRNG& prng) 
    //{ 
    ...
    //static inline
    //    void xmm_rand(span<__m128i> vec, oc::PRNG& prng) 
    //{
    //    byte_rand({ (uint8_t*)vec.data(), vec.size() * 16 }, prng); 
    //}
