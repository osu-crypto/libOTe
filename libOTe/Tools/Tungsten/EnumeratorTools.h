...
//#define MPZ_ENABLE
    ...
    Static Cache: The function uses a static cache, which is a std::vector of std::vector<T>, to store previously computed values of the binomial coefficient "n choose k." The cache allows the function to avoid recomputation by storing results of binomial coefficient calculations, which can be reused in future calls. The value -1 is used to denote uncomputed values within the cache.
