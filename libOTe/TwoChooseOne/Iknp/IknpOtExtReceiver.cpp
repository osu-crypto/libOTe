		...
		Socket& chl)
	{
		...
		// into the T1, T0 buffers for long term storage
		{
			auto t0 = AlignedUnVector<block>{ 128 };
			{
				...
				auto numOtExt = roundUpTo(choices.size(), 128);
				auto numBlocks = (numOtExt / 128);

				block* cIter = choices.blocks();
				...
				for (auto i = 0; i < numBlocks; ++i)
				{
					if (uIter == uEnd)
					{
						u64 step = std::min<u64>(numBlocks - i, (u64)commStepSize);
					...
					assert(mEnd <= messages.data() + messages.size());
					assert(tIter + size <= t0.data() + t0.size());

					memcpy(mIter, tIter, size * sizeof(block));
					mIter = mEnd;
				...
				assert(cIter == choices.blocks() + choices.sizeBlocks());
				assert(uIter == uEnd);
				assert(uBuff.size() == 0);
