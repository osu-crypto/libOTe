...
namespace osuCrypto
{
	...
	template<typename T> const size_t SoftSpokenShDotSenderWithVole<T>::commSize;
	template<typename T> const size_t SoftSpokenShDotReceiverWithVole<T>::commSize;

	// TODO: Maybe do something with the timer.

	template<typename SubspaceVole>
	void SoftSpokenShDotSenderWithVole<SubspaceVole>::setBaseOts(
		...
		bool malicious)
	{
		...
		mVole = std::make_unique<SubspaceVole>(
			SmallFieldVoleReceiver(
				fieldBits(), numVoles, chl, prng, baseRecvOts, choices, mNumThreads, malicious),
			RepetitionCode(numVoles)
			);

		mFieldBitsThenBlockIdx = 0;
		initTemporaryStorage();
	}

	template<typename SubspaceVole>
	void SoftSpokenShDotReceiverWithVole<SubspaceVole>::setBaseOts(
		...
		PRNG& prng, Channel& chl, bool malicious)
	{
		...
		const size_t numVoles = divCeil(gOtExtBaseOtCount, fieldBits());
		mVole = std::make_unique<SubspaceVole>(
			SmallFieldVoleSender(fieldBits(), numVoles, chl, prng, baseSendOts, mNumThreads, malicious),
			RepetitionCode(numVoles)
			);
	...
	template<typename SubspaceVole>
	void SoftSpokenShDotSenderWithVole<SubspaceVole>::send(
	...
	template<typename SubspaceVole>
	void SoftSpokenShDotSenderWithVole<SubspaceVole>::processChunk(
		size_t nChunk, size_t numUsed, span<std::array<block, 2>> messages)
	{
		...
		generateChosen(blockIdx, useAES(mVole->mVole.mNumVoles), span<block>(messagesPtr, wPadded()));
		xorMessages(numUsed, messagesPtr, messagesPtr);
	}

	// messagesOut and messagesIn must either be equal or non-overlapping.
	template<typename SubspaceVole>
	void SoftSpokenShDotSenderWithVole<SubspaceVole>::xorMessages(
	...
	template<typename SubspaceVole>
	void SoftSpokenShDotReceiverWithVole<SubspaceVole>::receive(
	...
	template<typename SubspaceVole>
	void SoftSpokenShDotReceiverWithVole<SubspaceVole>::processChunk(
		size_t nChunk, size_t numUsed, span<block> messages, block choices)
	{
		size_t blockIdx = mFieldBitsThenBlockIdx++;

		// Only 1 AES evaluation per VOLE is on a secret seed.
		generateChosen(blockIdx, useAES(mVole->mVole.mNumVoles), choices, messages);
	}

	template class SoftSpokenShDotSenderWithVole<SubspaceVoleReceiver<RepetitionCode>>;
	template class SoftSpokenShDotReceiverWithVole<SubspaceVoleSender<RepetitionCode>>;
	template class SoftSpokenShDotSenderWithVole<SubspaceVoleMaliciousReceiver<RepetitionCode>>;
	template class SoftSpokenShDotReceiverWithVole<SubspaceVoleMaliciousSender<RepetitionCode>>;
