...
namespace logvole::comm
{
    namespace
    {
        thread_local std::uint64_t g_thread_sender_wait_us = 0u;
        thread_local std::uint64_t g_thread_receiver_wait_us = 0u;
    }
    ...
    protocol_result<void> any_channel::send_frame(byte_buffer frame)
    {
        ...
        auto res = impl_->send_frame(std::move(frame));
        auto t1 = std::chrono::steady_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        if (config().role == role_t::sender)
        {
            global_timing_stats.sender_wait_time_us += elapsed;
            g_thread_sender_wait_us += static_cast<std::uint64_t>(elapsed);
        }
        else
        {
            global_timing_stats.receiver_wait_time_us += elapsed;
            g_thread_receiver_wait_us += static_cast<std::uint64_t>(elapsed);
        }
        return res;
    }
    ...
    protocol_result<any_channel> make_channel(const channel_config &config)
    {
        ...
        default:
            return protocol_result<any_channel>::failure(
                protocol_errc::unsupported_transport, "unknown transport kind");
        }
    }
    ...
    void reset_thread_local_wait_stats()
    {
        g_thread_sender_wait_us = 0u;
        g_thread_receiver_wait_us = 0u;
    }

    std::uint64_t read_thread_local_wait_us(role_t role)
    {
        return role == role_t::sender ? g_thread_sender_wait_us : g_thread_receiver_wait_us;
    }
