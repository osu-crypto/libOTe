...
namespace logvole
{
    ...
    comm::protocol_result<shrinkexpand_receiver_offline_output> run_shrinkexpand_receiver(
        comm::any_channel channel, const shrinkexpand_receiver_offline_input &input,
        const shrinkexpand_backend &backend);

    comm::protocol_result<shrinkexpand_shrink_output> shrinkexpand_shrink(
    ...
    comm::protocol_result<shrinkexpand_sender_expand_output> shrinkexpand_expand_sender(
        const shrinkexpand_sender_state &state, const shrinkexpand_expand_sender_input &input,
        const shrinkexpand_backend &backend);

    comm::protocol_result<shrinkexpand_receiver_expand_output> shrinkexpand_expand_receiver(
        const shrinkexpand_receiver_state &state, const shrinkexpand_expand_receiver_input &input,
        const shrinkexpand_backend &backend);
    ...
    using shrinkexpand_shrink_signature = comm::protocol_result<shrinkexpand_shrink_output> (*)(
    ...
    static_assert(
        std::is_same<decltype(&run_shrinkexpand_sender), run_shrinkexpand_sender_signature>::value,
        "run_shrinkexpand_sender signature mismatch");

    static_assert(
        std::is_same<decltype(&run_shrinkexpand_receiver), run_shrinkexpand_receiver_signature>::value,
        "run_shrinkexpand_receiver signature mismatch");

    static_assert(
        std::is_same<decltype(&shrinkexpand_shrink), shrinkexpand_shrink_signature>::value,
        "shrinkexpand_shrink signature mismatch");

    static_assert(
        std::is_same<decltype(&shrinkexpand_expand_sender), shrinkexpand_expand_sender_signature>::value,
        "shrinkexpand_expand_sender signature mismatch");

    static_assert(
        std::is_same<decltype(&shrinkexpand_expand_receiver), shrinkexpand_expand_receiver_signature>::value,
